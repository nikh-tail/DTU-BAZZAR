  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={onViewAll}
          className="group flex items-center gap-1.5 text-xs sm:text-sm font-bold text-emerald-700 hover:text-emerald-800 transition-colors"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
        {listings.length === 0
          ? [...Array(4)].map((_, i) => (
              <div
                key={i}
                className="h-64 sm:h-72 rounded-3xl bg-slate-200 border border-slate-300 animate-pulse"
              />
            ))
